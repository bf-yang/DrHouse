# Semantic filter (modal definition & training)
import pandas as pd
import os, random
...
def save_model(save_path, save_name, model):
    ...
    torch.save(model.state_dict(), os.path.join(save_path, save_name))#把模型的参数model.state_dict保存成一个字典
...
if __name__ == "__main__":
    ...
    """
        args: Only one magic number: 53, that is the max of length_counts.
    """
    best_dev_acc = 0
    epoch = 25
    batch_size = 32
    ...
    df = pd.read_excel("utils/adaptive_retrieval_dataset/adaptive_retrieval_dataset.xlsx", sheet_name='dataset')
